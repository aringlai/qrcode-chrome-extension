...
- **历史记录**: 自动保存生成记录，支持重命名、删除和批量清空
- **按需渲染**: 二维码按需生成，不存储图片数据，节省 95% 存储空间
- **存储监控**: 实时显示存储使用情况，支持颜色预警
...
- 存储限制: 5MB (Chrome 限制)
- 收藏上限: 100 条
- 每条记录约 200-500 字节（仅存储文本，不存储图片）

详细存储管理指南请参考 [STORAGE_GUIDE.md](./STORAGE_GUIDE.md)
